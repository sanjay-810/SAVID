...
from .savid_head_utils import GMAN,LSTM,LidarFusion

class savidHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        self.dif = GMAN(self.model_cfg.GMAN[0],self.model_cfg.GMAN[0],512,self.model_cfg.GMAN[1])
        self.asmn= LSTM(self.model_cfg.ASMN[0],self.model_cfg.ASMN[0])
        self.kgf = LidarFusion(self.model_cfg.ATTENTION_FC[0],self.model_cfg.ATTENTION_FC[0])
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['pseudo_features']=self.dif(batch_dict['multi_scale_3d_features'],batch_dict['gray_scale_3d_features'])
        # RoI Grid Pool
        ...
        x_valid = self.asmn(x_pseudo,batch_dict['pseudo_features'] )
        fusion_features = self.kgf(x_valid, x_pseudo)
